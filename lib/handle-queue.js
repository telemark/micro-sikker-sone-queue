...
module.exports.getAll = async (request, response) => {
  logger('info', ['handle-queue', 'getAll'])
  queue.find({isQueued: true}, (error, documents) => {
    if (error) {
      logger('error', ['handle-queue', 'getAll', error])
      send(response, 500, error)
    } else {
      if (documents.length > 0) {
        logger('info', ['handle-queue', 'getAll', 'documents', documents.length, 'success'])
      } else {
        logger('info', ['handle-queue', 'getAll', 'no documents in queue', 'success'])
      }
      send(response, 200, documents)
    }
  })
